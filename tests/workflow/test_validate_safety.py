...
def _build_summary_draft() -> DoctorFacingSummaryDraft:
...
def test_validate_safety_node_is_repeatable_across_restarts() -> None:
    service = SafetyService()
    node = ValidateSafetyNode(safety_service=service)
    draft = _build_summary_draft()

    first_result = node.validate(case_id="case_123", draft=draft)
    second_result = node.validate(case_id="case_123", draft=draft)

    assert first_result == second_result
    assert first_result == SafetyCheckResult(
        case_id="case_123",
        decision="pass",
        issues=(),
