...
def test_api_readiness_reports_not_ready_without_qdrant_collection(tmp_path: Path) -> None:
    ...
    assert any(
        dependency.name == "startup_verification"
        and dependency.status == "blocked"
        and dependency.reason_code == "qdrant_unreachable"
        for dependency in response.dependencies
    )
...
def test_local_profile_is_marked_degraded_even_when_dependencies_exist(tmp_path: Path) -> None:
    ...
    assert any(
        dependency.name == "startup_verification"
        and dependency.status == "degraded"
        and dependency.reason_code == "runtime_profile_local"
        for dependency in response.dependencies
    )
    assert all(
        dependency.status == "ready" or dependency.name == "startup_verification"
        for dependency in response.dependencies
    )
...
def test_startup_verification_passes_for_ready_operational_runtime(tmp_path: Path) -> None:
    ...
    assert response.status == StartupVerificationStatus.PASSED
    assert response.can_process_cases is True
    assert response.reason_codes == ()
...
def test_startup_verification_blocks_on_schema_compatibility_failure(
    ...
    assert response.can_process_cases is False
    assert "database_url_invalid" in response.reason_codes
    assert any(
        step.name == "schema_compatibility"
        and step.status == "blocked"
        and step.reason_code == "database_url_invalid"
        for step in response.steps
    )


def test_startup_verification_blocks_when_qdrant_collection_is_missing(
    tmp_path: Path,
) -> None:
    artifact_root_dir = tmp_path / "artifacts"
    knowledge_base_seed_dir = tmp_path / "knowledge-base"
    ...
    assert response.status == StartupVerificationStatus.BLOCKED
    assert response.can_process_cases is False
    assert "qdrant_collection_missing" in response.reason_codes
    assert any(
...
def test_startup_verification_reports_degraded_local_profile(tmp_path: Path) -> None:
    artifact_root_dir = tmp_path / "artifacts"
    knowledge_base_seed_dir = tmp_path / "knowledge-base"
    artifact_root_dir.mkdir()
    knowledge_base_seed_dir.mkdir()
    ...
    assert response.status == StartupVerificationStatus.DEGRADED
    assert response.can_process_cases is True
    assert "runtime_profile_local" in response.reason_codes
    assert any(
        step.name == "runtime_profile"
        and step.status == "degraded"
        and step.reason_code == "runtime_profile_local"
        for step in response.steps
    )
