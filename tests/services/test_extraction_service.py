...
def test_extract_indicators_discards_invalid_candidates_and_keeps_schema_safe_subset() -> None:
    now = datetime(2026, 4, 28, 6, 0, tzinfo=UTC)
    ...
    assert indicator_record is not None
    assert len(indicator_record.indicators) == 1
    assert indicator_record.indicators[0].name == "Hemoglobin"
    assert case_service.get_case_indicator_records(patient_case.case_id) == (indicator_record,)


def test_extract_indicators_returns_none_when_no_schema_safe_indicators_remain() -> None:
    now = datetime(2026, 4, 28, 6, 0, tzinfo=UTC)
