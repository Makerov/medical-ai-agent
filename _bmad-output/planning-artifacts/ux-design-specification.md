...
editHistory:
  - date: "2026-05-12"
    changes: "Added RU-first Real RAG UX pattern for source applicability, retrieval confidence, fallback downgrade, and operator trace details."
---
...
#### RU-first Real RAG UX Pattern

Epic 7 adds source-governed retrieval behavior that must be visible without overwhelming Telegram users.

Doctor-facing case card must distinguish:

- `high` retrieval confidence: applicable RU or allowed doctor-facing sources support the claim.
- `limited` retrieval confidence: support exists but has limitations, such as international fallback or narrow applicability.
- `ambiguous` retrieval confidence: multiple sources or applicability gates disagree.
- `insufficient` retrieval confidence: no acceptable source support is available.

Presentation rules:

- RU patient-facing sources should appear as primary support for Russian patient context.
- International fallback must be labeled as non-local support with a limitation note.
- Clinician-only sources may support doctor audit context but must not appear as direct patient instructions.
- ГРЛС must be shown only as registry/provenance context, not as advice, dosage or treatment instruction.
- Claims blocked or downgraded by source applicability must show a concise reason in doctor/operator surfaces.
- Missing or insufficient provenance must block normal `ready_for_doctor` presentation rather than appearing as a complete handoff.

Doctor-facing source details should use progressive disclosure:

1. Case card shows short confidence/source status next to relevant fact or summary claim.
2. Source details action opens citations, source class, jurisdiction, audience and limitation note.
3. Audit/artifact action opens retrieval run ID and trace metadata for operator review.

Operator-facing artifact display should include:

- retrieval run ID;
- active Qdrant alias;
- physical collection/index version;
- embedding provider metadata summary;
- selected/rejected chunk counts;
- applicability decision reason codes;
- downgrade/block reason;
- sensitive-payload minimization status.

Telegram copy should avoid technical noise in the main card. Detailed alias, collection and embedding metadata belong in artifact/detail views, not the primary doctor summary.
