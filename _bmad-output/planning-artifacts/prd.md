...
Основная аудитория проекта - интервьюер или нанимающий AI lead, которому нужно быстро увидеть зрелость AI engineering: agents, RAG, structured outputs, document processing, safety, evals, observability и продуктовые trade-offs в чувствительном домене. Пользовательские роли внутри продукта - пациент, создающий обращение, и врач, принимающий подготовленный case handoff.

### What Makes This Special

Ключевое отличие проекта - фокус на безопасном human-in-the-loop workflow вместо демонстрации "AI-доктора". Система не ставит диагноз, не назначает лечение и не выдает клиническое решение как финальный ответ. Она подготавливает проверяемый intake package с источниками, ограничениями уверенности и явной передачей врачу.

Проект демонстрирует end-to-end AI system design, а не одиночный chatbot: отдельные роли для patient intake, document extraction, medical summarization, RAG grounding, safety checking и doctor handoff объединены в backend workflow. RAG-слой и provenance источников позволяют показывать, на чем основаны подсветка показателей и summary, а evals проверяют extraction quality, groundedness и safety behavior.

Core insight: в healthtech portfolio убедительнее показать ответственную AI-автоматизацию вокруг врача, чем имитировать автономную диагностику. Такой подход демонстрирует инженерную зрелость в regulated-domain AI: минимизация риска, явное согласие, audit trail, role separation и ограничение модели подготовкой информации.

## Project Classification

**Project Type:** `api_backend` с Telegram bot интерфейсами. Основная ценность находится в backend workflow: FastAPI, aiogram, LangGraph orchestration, document parsing, structured outputs, RAG, storage, evals и observability.
...
**Project Context:** `greenfield`. PRD описывает новый продукт без существующей production-системы или brownfield constraints.
...
Проект успешен как портфолио, если интервьюер или AI lead за 5-10 минут может понять архитектуру, пройти demo happy path и увидеть зрелые engineering decisions вокруг agents, RAG, structured outputs, safety, evals и observability.

Демо должно показывать не только UX ботов, но и backend workflow: state machine, document processing, RAG grounding, safety layer, audit trail и doctor handoff.

### Technical Success

Система извлекает показатели из тестового медицинского документа в structured JSON с единицами измерения, референсными диапазонами, provenance и отметками неопределенности.

RAG-слой использует контролируемую knowledge base с источниками, ссылками, датами доступа и областью применимости. Summary для врача должно быть grounded в извлеченных фактах и источниках.

Safety layer блокирует диагнозы, назначения лечения и чрезмерно уверенные формулировки. Doctor-facing AI outputs должны сохранять достаточный audit trail для demo: исходные документы или ссылки на них, использованные источники, case ID и результат safety check.

### Measurable Outcomes

- Время от завершения загрузки документов до готовой карточки врача: практичное demo window, пригодное для интервью и задокументированное в README.
- Доля успешно извлеченных ключевых показателей из подготовленного demo-документа: >= 85%.
...
- Happy path demo должен проходить end-to-end без ручного вмешательства разработчика.
- README должен позволять интервьюеру запустить demo локально через Docker Compose и понять ключевые trade-offs.
...
- OCR/document parsing для одного-двух типов demo-документов: PDF и/или изображение анализа.
...
- Docker Compose, README, seed data и архитектурная схема.
...
Medical AI Agent может стать демонстрационной платформой для AI-assisted medical intake: поддерживать разные направления чек-апов, сравнивать модели, запускать offline evals, хранить synthetic medical cases и показывать качество AI pipeline через dashboard.
...
Если пациент не может предоставить лучший файл, система продолжает обработку только по reliable facts и маркирует uncertainty в карточке врача. Для пациента успешный исход - понятное восстановление после ошибки без технических деталей и без ложной уверенности.
...
Если врач видит questionable extraction или low-confidence field, карточка должна явно показывать uncertainty и ссылку на исходный документ. Врач может открыть исходные материалы и самостоятельно проверить спорный показатель.
...
### Journey 4: Interviewer / AI Lead Demo - Оценить инженерную зрелость за 5-10 минут

Нанимающий AI lead открывает проект как portfolio artifact. Его цель - быстро понять, умеет ли кандидат строить не только LLM wrapper, а полноценную AI-систему в чувствительном домене.

Он запускает demo через Docker Compose, проходит happy path: patient submits documents, backend processes case, doctor receives prepared summary. Затем он смотрит README, архитектурную схему, seed knowledge base, examples of structured outputs, eval results и safety behavior.

Ключевой момент успеха - AI lead видит обоснованные engineering trade-offs: LangGraph workflow, RAG grounding, structured schemas, safety pass, source provenance, selected demo artifacts и tests. Проект выглядит как реалистичный backend system, а не как одноразовый prompt demo.

Эта journey раскрывает требования к reproducible local setup, demo data, README, architecture diagram, eval reports, observability examples и clear portfolio narrative.

### Journey 5: Developer / Operator - Объяснить происхождение summary в demo

Разработчик или интервьюер замечает, что summary выглядит слишком уверенным или ссылается на спорный reference range. Ему нужно понять, какие входные данные, extracted facts, источники и safety decision привели к doctor-facing summary.

Он открывает demo artifacts по `case_id`: структурированное извлечение, использованные источники, итоговый summary и результат safety check. Если проблема в knowledge base или extraction, она становится видимой как limitation или future improvement, а не скрывается за непрозрачным AI output.

Успех этой journey - portfolio reviewer может проверить происхождение summary без полноценной production observability платформы. MVP показывает достаточную traceability для demo, но не обещает полный replay/debug workflow.

Эта journey раскрывает требования к stable case ID, selected demo artifacts, source provenance, safety decisions и minimal eval evidence.
...
- Demo readiness: Docker Compose, seed cases, README, architecture diagram, reproducible happy path.
- Observability and quality: stable case IDs, selected demo artifacts, source provenance, safety decisions, minimal eval evidence.

## Domain-Specific Requirements

### Compliance & Regulatory

Проект ориентирован только на российский медицинский контекст. В MVP он должен позиционироваться как portfolio/demo system для подготовки медицинского обращения, а не как медицинское изделие, система поддержки принятия врачебных решений или сервис оказания медицинской помощи.

Все формулировки в `patient_bot`, `doctor_bot`, README и demo должны поддерживать границу: AI извлекает, структурирует и подготавливает информацию для врача; врач принимает медицинское решение. Система не должна ставить диагноз, назначать лечение, интерпретировать состояние пациента как финальное медицинское заключение или подменять консультацию врача.
...
Если функциональность начнет выполнять медицинское назначение, влиять на clinical decision-making или использоваться как СППВР, потребуется отдельная оценка статуса ПО как медицинского изделия и возможной регистрации в контуре Росздравнадзора. MVP должен явно оставаться вне этой зоны: demo intake assistant, не медицинское изделие.

### Technical Constraints

Система должна хранить минимально необходимый объем персональных и медицинских данных. Для demo предпочтительны synthetic cases или обезличенные документы; реальные медицинские документы не должны использоваться без отдельного согласия, правового основания и политики обработки.

Consent flow должен явно объяснять: какие данные собираются, зачем они нужны, кому передаются, как долго хранятся, как удалить кейс и что AI не оказывает медицинскую помощь. Для demo-сценария consent должен быть продуктовым и демонстрационным, без заявления о полноценной юридической готовности к production.
...
Medical documents, extracted facts, summaries и selected demo artifacts должны быть связаны с `case_id`, чтобы можно было объяснить путь от исходного документа до doctor-facing summary.
...
Telegram используется как demo UX channel. Core workflow должен быть отделен от Telegram: patient intake, document processing, RAG, safety check и doctor handoff должны оставаться backend capabilities, которые позже можно подключить к web dashboard или медицинской информационной системе.

Любая будущая интеграция с медицинской организацией, МИС, ЕГИСЗ-контуром или реальными пациентскими данными требует отдельного legal/security review и уточнения роли системы: demo assistant, внутренний инструмент врача, СППВР или медицинское изделие.
...
Mitigation: synthetic/обезличенные demo cases, минимизация хранения, deletion flow, access control, audit trail, отдельная production compliance assessment.

Риск: Telegram как канал не соответствует ожиданиям production medical data handling.  
Mitigation: использовать Telegram только для portfolio/demo, не заявлять production readiness, вынести core processing в backend boundaries.

Риск: OCR или extraction ошибочно распознает показатель.  
Mitigation: confidence scoring, uncertainty markers, ссылка на исходный документ, retry flow для плохого качества, partial processing только reliable facts.

Риск: RAG подбирает неподходящий источник или reference range.  
Mitigation: curated knowledge base, applicability metadata, provenance display, evals на groundedness, regression tests для известных кейсов.

## Innovation & Novel Patterns

### Detected Innovation Areas

Главная инновационная зона проекта - не новый медицинский алгоритм, а композиция AI workflow для безопасного medical intake: Telegram intake, OCR/document parsing, structured extraction, RAG grounding, safety validation, audit trail и doctor handoff объединены в один демонстрационный backend pipeline.

Вторая зона - portfolio positioning. Проект сознательно отказывается от "AI doctor" framing и показывает более зрелую модель: AI подготавливает проверяемый intake package, а врач остается decision maker. Это делает проект сильнее для собеседования, потому что демонстрирует работу с regulated-domain constraints, а не только prompt engineering.
...
Проект не претендует на конкуренцию с production МИС, телемедицинскими платформами или зарегистрированными СППВР. Его контекст - portfolio/demo artifact для демонстрации AI engineering maturity.

Альтернативы в portfolio-среде обычно выглядят как простые chatbots или RAG demos. Medical AI Agent отличается тем, что показывает end-to-end system boundaries: два Telegram-интерфейса, backend orchestration, structured schemas, provenance, safety checks, evals и observability.

### Validation Approach

Innovation value проверяется через demo и evals:
...
- demo review: AI lead может за 5-10 минут понять архитектуру и trade-offs.

### Risk Mitigation

Риск: инновационность выглядит как "просто Telegram bot с LLM".  
Mitigation: README и demo должны показывать backend workflow, schemas, traces, evals, safety layer и source provenance.
...
Mitigation: описывать новизну скромно и инженерно: novel composition for portfolio demonstration, not a new clinical method.
...
Telegram bots должны быть тонкими интерфейсами поверх backend capabilities. Core workflow не должен быть жестко связан с Telegram, чтобы позже можно было подключить web dashboard, demo CLI или другой UI без переписывания AI pipeline.
...
- `audit and observability`: stable case IDs, source provenance, safety decisions, selected demo artifacts, eval fixtures.
...
- retrieving selected demo artifacts for portfolio review and evaluation.

These endpoints are primarily consumed by `patient_bot`, `doctor_bot`, background workers and demo/eval tooling. Public third-party API access is out of MVP scope.

### Auth Model

MVP authentication should be pragmatic and demo-safe:
...
These limits protect demo reliability and make failure behavior predictable.

### API Docs

The project should expose generated OpenAPI docs for backend routes used by bots and demo tooling. README should include example request/response payloads for the core case lifecycle and examples of structured extraction, safety check and doctor summary outputs.
...
**MVP Approach:** portfolio demo MVP. Первая версия должна доказать end-to-end AI engineering capability: пациент создает обращение, документы обрабатываются backend pipeline, врач получает AI-prepared case card, а интервьюер видит архитектуру, safety boundaries, evals и observability.

**Resource Requirements:** один AI/backend engineer может реализовать MVP при ограниченном scope. Минимальные навыки: Python/FastAPI, aiogram, LangGraph или эквивалентная orchestration модель, PostgreSQL, pgvector/RAG, document parsing/OCR, LLM structured outputs, Docker Compose, тестирование и evals.
...
- Interviewer demo: воспроизводимый local happy path с README, seed data и архитектурной схемой.
- Developer/operator review: stable case IDs, selected demo artifacts и eval fixtures для объяснения pipeline.
...
- OCR/document parsing для ограниченного набора demo-документов.
...
- Basic auditability: `case_id`, source provenance, safety decisions, selected demo artifacts.
- Minimal eval suite для extraction, groundedness и safety.
- Docker Compose, seed demo case, README и architecture diagram.
...
- Более строгая access control модель для demo/admin/debug функций.
...
- Synthetic medical case library для offline evals и сравнения моделей.

### Risk Mitigation Strategy

**Technical Risks:** самый высокий риск - качество OCR/extraction и groundedness в медицинском контексте. MVP снижает риск через ограниченный набор demo-документов, typed schemas, confidence markers, source provenance, eval fixtures и ручную проверяемость doctor-facing outputs.

**Market/Positioning Risks:** главный риск для portfolio - проект может выглядеть как небезопасный "AI doctor" или как обычный Telegram bot. MVP снижает риск через clear non-goals, human-in-the-loop framing, safety layer, architecture diagram, traces и eval results.
...
### Готовность demo и maintainability
